...
def Network(convShape,shape,e,r,m):#convShape = array, (convLayers X 2), [numFilters,filterSize] // shape = array, (Layers)
    ...
    LFilter = np.random.normal(loc = 0, scale = stddev, size = (convShape[0][0],convShape[0][1],convShape[0][1], X[0].shape[-1]))
    ...
    while(itt < len(convShape)):#for each layer after the first
            ...
            filters.append(LFilter)
            FMPools.append(pool(relu(conv(FMPools[-1],filters[-1]))))
            itt += np.intc(1)
    flatFMPool = (FMPools[-1].reshape((FMPools[-1].size,1)))


    print('Xs shape: ',X.shape)
    w0 = np.random.standard_normal(size=(shape[1],flatFMPool.shape[0])).astype(np.float64) * np.float64(0.01)
    ...
    W, y = fit(X,T,W,B,E,R,L, convShape, shape, hfinal, h, hPrime, loss, M, filters, batchSize)   
    print(y[-1])
    return W, y
...
def fit(X,T,W,B,E,R,L,convShape,shape,hfinal,h,hP,loss,M,filters, batchSize):
    xplt = np.arange(E)
    ...
    WincPrev = np.float64(0)

    while(itt<E):#training loop
        shuffled_index = np.random.permutation(X.shape[0])
        batch_train_X = X[shuffled_index[:batchSize]]
        batch_train_Y = T[shuffled_index[:batchSize]]
        flatFMpools, FMPools = ArbConvolve(batch_train_X,convShape,filters)
        Z = forwardProp(flatFMpools,W,B,hfinal,h,L,shape)
        W, B, yplt, err, WincPrev = backProp(Z,batch_train_Y,R,hP,W,B,L,shape,itt,yplt,loss,M,WincPrev)
        convBackProp(FMPools, filters, err, R)
...
def ArbConvolve(imgs, convShape, filters):#convShape = ndarray, (convLayers X 2), [numFilters,filterSize]
    ...
    while(it < imgs.shape[0]):#for num of imgs
        print("img:", it)
        FMPools.append(pool(relu(conv(imgs[it],filters[0]))))
        itt=np.intc(1)
        while(itt < len(filters)):#for each layer
            FMPools.append(pool(relu(conv(FMPools[-1],filters[itt]))))
            itt += np.intc(1)
...
def conv(img, conv_filter):
    ...
    feature_maps = np.zeros((img.shape[0]-conv_filter.shape[1]+1, 
                                ...
                                conv_filter.shape[0], img.shape[-1]))
    ...
    for filter_num in range(conv_filter.shape[0]):
        ...
        feature_maps[:, :, filter_num, 0] = conv_map # Holding feature map with the current filter.
...
def backProp(y,T,R,hP,W,B,L,shape,itt,yplt,loss,M, WincPrev):
    w=W
    b=B
    ...
    while(itter>=0):
        ...
        np.add(np.add(w[itter], Wincrement,out=w[itter]), (M - 1) * WincPrev, out = w[itter])
        iterator=np.intc(0)
        ...
        dlta.append(err)
        itter-=np.intc(1)
    return w, b, yplt, err, Wincrement

def convBackProp(featureMap, W, err, Rate):
    Layers = len(W)
